...
export class SearchService {
  ...
  async search(organizationId: string, rawQuery: string, limit: number) {
    ...
    if (q.length < 2) throw new BadRequestException('Search query must contain at least 2 characters');
    const like = `%${q.replace(/[%_]/g, '\\$&')}%`;
    const rows = await this.prisma.$queryRawUnsafe<Array<{
      id: string; type: string; title: string; subtitle: string | null; href: string | null; rank: number;
    }>>(
      `SELECT * FROM (
