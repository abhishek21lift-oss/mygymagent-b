...
/**
 * Execution layer for assigned workout plans: starting a session from an
 * active assignment, logging sets per exercise as they happen, and
 * completing the session. Exercises are snapshotted from the assigned plan
 * at start time (WorkoutSession.exercises), so later edits to the plan
 * never rewrite execution history -- the same immutability convention the
 * rest of this codebase applies to financial and historical records.
 *
 * Tenant isolation follows the standard pattern: every query is scoped by
 * `organizationId` from the verified JWT. For trainers whose access comes
 * through `workouts.read_assigned` (the `CurrentAssignmentScope`
 * decorator), reads are additionally scoped to the caller's own assigned
 * members -- a trainer can never open or mutate another trainer's
 * session execution.
 */
@Injectable()
export class WorkoutSessionsService {
  ...
  private static readonly EXERCISE_SNAPSHOT_KEYS = [
    'id',
    'exerciseId',
    'name',
    'setsTarget',
  ...
  async start(
    ...
    if (assignment.status !== 'ACTIVE') {
      throw new BadRequestException(
        'Only an active assignment can start a workout session',
      );
    }
    ...
    // Resolve exercise names from the org's exercise library (the plan
    // JSON only stores exerciseId/order/sets/reps -- the name is owned by
    // the library entry), falling back to a stored name if one exists.
    const referencedIds = planExercises
      .filter((entry) => typeof entry.exerciseId === 'string')
      .map((entry) => entry.exerciseId as string);
    const libraryExercises = await this.prisma.exercise.findMany({
      where: { organizationId, id: { in: referencedIds } },
      select: { id: true, name: true },
    });
    const nameById = new Map(
      libraryExercises.map((exercise) => [exercise.id, exercise.name]),
    );
    ...
    const snapshot = planExercises.map((entry, index) => ({
      ...
      name:
        (typeof entry.exerciseId === 'string' &&
          nameById.get(entry.exerciseId)) ||
        (typeof entry.name === 'string' ? entry.name : 'Exercise'),
      setsTarget: typeof entry.sets === 'number' ? entry.sets : null,
      repsTarget:
        typeof entry.reps === 'string' || typeof entry.reps === 'number'
          ? String(entry.reps)
          : null,
      restSeconds:
    ...
    const session = await this.prisma.workoutSession.create({
      data: {
        ...
        branchId: await this.resolveMemberBranch(
          organizationId,
          assignment.memberId,
        ),
        exercises: snapshot,
  ...
  async logSet(
    ...
    if (session.status !== 'IN_PROGRESS') {
      throw new BadRequestException(
        'Cannot log sets to a completed workout session',
      );
    }

    const exercises = Array.isArray(session.exercises)
      ? (session.exercises as unknown as Array<{ id?: unknown }>)
      : [];
    if (!exercises.some((entry) => entry.id === sessionExerciseId)) {
      throw new BadRequestException(
        'Exercise is not part of this workout session',
      );
    }
  ...
  async complete(
    ...
    const event: WorkoutSessionCompletedEvent = {
      ...
      workoutAssignmentId: updated.assignmentId,
  ...
  /** The member's current primary branch is the session's branch. */
  private async resolveMemberBranch(
    organizationId: string,
    memberId: string,
  ): Promise<string> {
  ...
  private toSessionResponse(session: {
    ...
    assignment: { workoutPlan: { name: string } };
    sets: Array<{
      id: string;
      exerciseId: string;
      setNumber: number;
    ...
    const exercises = (
      Array.isArray(session.exercises)
        ? (session.exercises as unknown as Array<
            Record<
              (typeof WorkoutSessionsService.EXERCISE_SNAPSHOT_KEYS)[number],
              unknown
            >
          >)
        : []
      ...
      exerciseName: entry.name ?? 'Exercise',
      setsTarget: entry.setsTarget ?? null,
      repsTarget: entry.repsTarget ?? null,
