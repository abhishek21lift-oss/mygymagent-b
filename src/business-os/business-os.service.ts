...
export class BusinessOsService {
  ...
  async loyaltyAdjust(
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      await tx.$queryRawUnsafe(
        'INSERT INTO loyalty_accounts(organization_id,member_id) VALUES($1,$2) ON CONFLICT (organization_id,member_id) DO NOTHING',
        org,
        memberId,
      );
      const rows = await tx.$queryRawUnsafe<any[]>(
        "UPDATE loyalty_accounts SET points=GREATEST(points+$1,0), tier=CASE WHEN GREATEST(points+$1,0)>=5000 THEN 'PLATINUM' WHEN GREATEST(points+$1,0)>=2000 THEN 'GOLD' WHEN GREATEST(points+$1,0)>=500 THEN 'SILVER' ELSE 'STANDARD' END, updated_at=now() WHERE organization_id=$2 AND member_id=$3 RETURNING *",
        points,
        org,
        memberId,
      );
      ...
      await tx.$executeRawUnsafe(
        'INSERT INTO loyalty_ledger(organization_id,member_id,points,reason) VALUES($1,$2,$3,$4)',
        org,
        memberId,
        points,
        s(reason, 'Manual adjustment'),
      );
      return rows[0];
    });
  ...
  async createReferral(org: string, referrerId: string) {
    ...
    await this.prisma.$executeRawUnsafe(
      'INSERT INTO referrals(organization_id,referrer_member_id,code) VALUES($1,$2,$3)',
      org,
      referrerId,
      code,
    );
  ...
  async convertReferral(org: string, id: string, referredMemberId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const rows = await tx.$queryRawUnsafe<any[]>(
        "UPDATE referrals SET referred_member_id=$1,status='CONVERTED',converted_at=now() WHERE id=$2 AND organization_id=$3 AND status='PENDING' RETURNING *",
        referredMemberId,
        id,
        org,
      );
      ...
      if (reward > 0) {
        ...
        await tx.$queryRawUnsafe(
          "UPDATE loyalty_accounts SET points=GREATEST(points+$1,0), tier=CASE WHEN GREATEST(points+$1,0)>=5000 THEN 'PLATINUM' WHEN GREATEST(points+$1,0)>=2000 THEN 'GOLD' WHEN GREATEST(points+$1,0)>=500 THEN 'SILVER' ELSE 'STANDARD' END, updated_at=now() WHERE organization_id=$2 AND member_id=$3",
          reward,
          org,
          referral.referrer_member_id,
        );
        await tx.$executeRawUnsafe(
          'INSERT INTO loyalty_ledger(organization_id,member_id,points,reason,reference_type,reference_id) VALUES($1,$2,$3,$4,$5,$6)',
          org,
          referral.referrer_member_id,
          reward,
          'Referral conversion',
  ...
  async createTicket(org: string, userId: string, b: any) {
    ...
    if (memberId) {
      ...
      if (branchId && member.primaryBranchId !== branchId)
        throw new BadRequestException(
          'Member does not belong to the selected branch',
        );
    }
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
  ...
  async accountingJournal(org: string, userId: string, b: any) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const l of lines) {
        const d = n(l.debit),
          cr = n(l.credit);
        if ((d <= 0 && cr <= 0) || (d > 0 && cr > 0))
          throw new BadRequestException(
            'each journal line must have exactly one positive side',
          );
        const account = await tx.$queryRawUnsafe<any[]>(
          'SELECT id FROM accounting_accounts WHERE id=$1 AND organization_id=$2 AND active=true',
          l.accountId,
          org,
        );
        if (!account[0])
        ...
        if (l.branchId) {
          const branch = await tx.$queryRawUnsafe<any[]>(
        ...
        const rows = await tx.$queryRawUnsafe<any[]>(
          'INSERT INTO accounting_entries(organization_id,account_id,branch_id,reference_type,reference_id,debit,credit,description,entry_date) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',
          org,
          l.accountId,
          l.branchId ?? null,
          ...
          d,
          cr,
          s(l.description, 'Journal entry'),
          b.entryDate ? new Date(b.entryDate) : new Date(),
        );
        created.push(rows[0]);
      }
      await this.audit.record({
        organizationId: org,
        actorUserId: userId,
        action: 'ACCOUNTING_JOURNAL_CREATE',
        resource: 'accounting_journal',
        afterState: { lines: created },
      });
      return created;
    });
  }
  ...
  async createCampaign(org: string, b: any) {
    ...
    if (!['EMAIL', 'WHATSAPP', 'SMS'].includes(channel))
      throw new BadRequestException('channel must be EMAIL, WHATSAPP or SMS');
    if (b.branchId) await this.ensureBranch(org, String(b.branchId));
    const audienceFilter =
      b.audienceFilter && typeof b.audienceFilter === 'object'
        ? b.audienceFilter
        : {};
    return this.prisma.$queryRawUnsafe(
  ...
  async enrollCampaign(org: string, id: string) {
    const camp = await this.prisma.$queryRawUnsafe<any[]>(
      'SELECT * FROM marketing_campaigns WHERE id=$1 AND organization_id=$2',
      id,
      org,
    );
    ...
    try {
      filter = camp[0].audience_filter
        ? typeof camp[0].audience_filter === 'string'
          ? JSON.parse(camp[0].audience_filter)
          : camp[0].audience_filter
        : {};
    } catch {
      throw new BadRequestException('Invalid campaign audience filter');
    }
    ...
    for (const key of Object.keys(filter))
      if (!allowed.has(key))
        throw new BadRequestException('Unsupported audience filter: ' + key);
    ...
    if (filter.branchId) {
      await this.ensureBranch(org, String(filter.branchId));
      add('m.primary_branch_id=$X', String(filter.branchId));
    }
    ...
    if (filter.assignedTrainerId) {
      const trainer = await this.prisma.user.findFirst({
        where: {
          id: String(filter.assignedTrainerId),
          organizationId: org,
          deletedAt: null,
        },
        select: { id: true },
    ...
    if (filter.hasEmail === true)
      where.push("m.email IS NOT NULL AND btrim(m.email) <> ''");
    if (filter.hasPhone === true)
      where.push("m.phone IS NOT NULL AND btrim(m.phone) <> ''");
    if (filter.hasActiveMembership === true)
      where.push(
        "EXISTS (SELECT 1 FROM memberships ms WHERE ms.organization_id=m.organization_id AND ms.member_id=m.id AND ms.status='ACTIVE' AND ms.end_date >= CURRENT_DATE)",
      );
    if (filter.hasActiveMembership === false)
      where.push(
        "NOT EXISTS (SELECT 1 FROM memberships ms WHERE ms.organization_id=m.organization_id AND ms.member_id=m.id AND ms.status='ACTIVE' AND ms.end_date >= CURRENT_DATE)",
      );
    if (filter.minDaysSinceCheckIn !== undefined) {
      const min = Number(filter.minDaysSinceCheckIn);
      if (!Number.isInteger(min) || min < 0)
        throw new BadRequestException(
          'minDaysSinceCheckIn must be a non-negative integer',
        );
      params.push(min);
      const ph = '$' + params.length;
      where.push(
        "(NOT EXISTS (SELECT 1 FROM attendances ax WHERE ax.organization_id=m.organization_id AND ax.member_id=m.id) OR EXISTS (SELECT 1 FROM attendances a WHERE a.organization_id=m.organization_id AND a.member_id=m.id GROUP BY a.member_id HAVING MAX(a.check_in_at) <= CURRENT_TIMESTAMP - INTERVAL '1 day' * " +
          ph +
          '))',
      );
    }
    if (filter.maxDaysSinceCheckIn !== undefined) {
      ...
      if (!Number.isInteger(max) || max < 0)
        throw new BadRequestException(
          'maxDaysSinceCheckIn must be a non-negative integer',
        );
      params.push(max);
      const ph = '$' + params.length;
      where.push(
    ...
    const members = await this.prisma.$queryRawUnsafe<any[]>(
      'SELECT m.id FROM members m WHERE ' +
        where.join(' AND ') +
    ...
    for (const m of members)
      await this.prisma.$executeRawUnsafe(
        'INSERT INTO marketing_campaign_members(organization_id,campaign_id,member_id) VALUES($1,$2,$3) ON CONFLICT DO NOTHING',
        org,
        id,
        m.id,
      );
    await this.prisma.$executeRawUnsafe(
      "UPDATE marketing_campaigns SET status='QUEUED',updated_at=now() WHERE id=$1 AND organization_id=$2 AND status IN ('DRAFT','QUEUED','RUNNING')",
      id,
      org,
    );
  ...
  async runCampaign(org: string, id: string) {
    ...
    if (!camp[0]) throw new NotFoundException('Campaign not found');
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      "SELECT c.member_id,m.phone,m.email FROM marketing_campaign_members c JOIN members m ON m.id=c.member_id WHERE c.campaign_id=$1 AND c.organization_id=$2 AND c.status='QUEUED' LIMIT 500",
      id,
      org,
    );
    let sent = 0,
      failed = 0;
    for (const r of rows) {
      const channel = ['WHATSAPP', 'SMS', 'EMAIL'].includes(camp[0].channel)
        ? camp[0].channel
        : 'EMAIL';
      const recipient =
        channel === 'WHATSAPP' || channel === 'SMS' ? r.phone : r.email;
      if (!recipient) {
        failed++;
        await this.prisma.$executeRawUnsafe(
          "UPDATE marketing_campaign_members SET status='FAILED',error=$3 WHERE campaign_id=$1 AND member_id=$2 AND status='QUEUED'",
          id,
          r.member_id,
      ...
      try {
        await this.communications.sendAdHoc({
          ...
          category: 'MARKETING',
          recipient,
          body: camp[0].template_key ?? camp[0].name,
        });
        await this.prisma.$executeRawUnsafe(
          "UPDATE marketing_campaign_members SET status='SENT',sent_at=now(),error=NULL WHERE campaign_id=$1 AND member_id=$2 AND status='QUEUED'",
          id,
          r.member_id,
        );
        sent++;
    ...
    await this.prisma.$executeRawUnsafe(
      "UPDATE marketing_campaigns SET status=CASE WHEN EXISTS (SELECT 1 FROM marketing_campaign_members WHERE campaign_id=$1 AND organization_id=$2 AND status='QUEUED') THEN 'RUNNING' ELSE 'COMPLETED' END,updated_at=now() WHERE id=$1 AND organization_id=$2",
      id,
      org,
    );
  ...
  createAccount(org: string, b: any) {
    ...
    if (!allowedTypes.has(type))
      throw new BadRequestException('Invalid accounting account type');
    return this.prisma.$queryRawUnsafe(
  ...
  async entry(org: string, userId: string, b: any) {
    const debit = n(b.debit),
      credit = n(b.credit);
    if ((debit <= 0 && credit <= 0) || (debit > 0 && credit > 0))
      throw new BadRequestException(
        'exactly one of debit or credit must be positive',
      );
    throw new BadRequestException(
      'Use /accounting/journal for posting balanced accounting transactions',
    );
  }
  ...
  async createPortalInvite(org: string, userId: string, memberId: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.$executeRawUnsafe(
        'UPDATE portal_invites SET used_at=COALESCE(used_at,now()) WHERE organization_id=$1 AND member_id=$2 AND used_at IS NULL',
        org,
        memberId,
      );
      await tx.$executeRawUnsafe(
        "INSERT INTO portal_invites(organization_id,member_id,token_hash,expires_at) VALUES($1,$2,$3,now()+interval '7 days')",
        org,
        memberId,
        hash(token),
      );
  ...
  async portalBootstrap(token: string, clientKey: string) {
    if (!token || token.length < 32)
      throw new BadRequestException('Invalid portal token');
    await this.rateLimit('portal-bootstrap', clientKey || 'unknown', 20, 60);
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      'SELECT p.*,m.first_name,m.last_name,m.email,m.phone,m.primary_branch_id FROM portal_invites p JOIN members m ON m.id=p.member_id WHERE p.token_hash=$1 AND p.used_at IS NULL AND p.expires_at>now() AND m.deleted_at IS NULL',
      hash(token),
    );
    if (!rows[0])
    ...
    const consumed = await this.prisma.$executeRawUnsafe(
      'UPDATE portal_invites SET used_at=now() WHERE id=$1 AND used_at IS NULL AND expires_at>now()',
      r.id,
    );
    if (Number(consumed) !== 1)
      throw new NotFoundException('Invalid or expired portal token');
    const memberships = await this.prisma.membership.findMany({
      where: { organizationId: r.organization_id, memberId: r.member_id },
  ...
  async registerKiosk(org: string, userId: string, b: any) {
    ...
    await this.audit.record({
      ...
      afterState: { branchId: b.branchId, name: s(b.name) },
    });
  ...
  async kioskCheckin(deviceKey: string, memberId: string, clientKey: string) {
    ...
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      'SELECT * FROM kiosk_devices WHERE key_hash=$1 AND active=true',
      hash(deviceKey),
    );
    const d = rows[0];
    if (!d) throw new BadRequestException('Invalid kiosk key');
    const member = await this.prisma.member.findFirst({
      where: {
        id: memberId,
    ...
    if (!member) {
      await this.prisma.$executeRawUnsafe(
        "INSERT INTO kiosk_events(organization_id,branch_id,device_id,member_id,event_type,result) VALUES($1,$2,$3,$4,'CHECK_IN','DENIED')",
        d.organization_id,
        d.branch_id,
        d.id,
        memberId,
    ...
    if (member.primaryBranchId !== d.branch_id) {
      await this.prisma.$executeRawUnsafe(
        "INSERT INTO kiosk_events(organization_id,branch_id,device_id,member_id,event_type,result) VALUES($1,$2,$3,$4,'CHECK_IN','DENIED')",
        d.organization_id,
        d.branch_id,
    ...
    const decision = await this.attendance.evaluateGate(
      d.organization_id,
      memberId,
    );
