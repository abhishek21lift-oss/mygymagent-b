...
export class BusinessOsService {
  ...
  async accountingJournal(org:string,userId:string,b:any){
    const lines: Array<{accountId:string;debit?:unknown;credit?:unknown;branchId?:string;description?:string;}> = Array.isArray(b.lines)?b.lines:[]; if(lines.length<2) throw new BadRequestException('at least two journal lines are required');
