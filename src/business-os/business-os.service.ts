...
export class BusinessOsService {
  ...
  async loyaltyAdjust(
    ...
    const updated = await this.prisma.$queryRawUnsafe<any[]>(
      "UPDATE loyalty_accounts SET points=GREATEST(points+$1,0), tier=CASE WHEN points+$1>=5000 THEN 'PLATINUM' WHEN points+$1>=2000 THEN 'GOLD' WHEN points+$1>=500 THEN 'SILVER' ELSE 'STANDARD' END, updated_at=now() WHERE organization_id=$2 AND member_id=$3 RETURNING *",
      points,
      org,
      memberId,
    );
  ...
  async convertReferral(org: string, id: string, referredMemberId: string) {
    ...
    if (reward > 0)
      await this.loyaltyAdjust(
        org,
        'system',
        rows[0].referrer_member_id,
        reward,
        'Referral conversion',
      );
    return rows[0];
  ...
  async accountingJournal(org: string, userId: string, b: any) {
    ...
    const debit = lines.reduce((a, l) => a + n(l.debit), 0),
      credit = lines.reduce((a, l) => a + n(l.credit), 0);
    if (Math.abs(debit - credit) > 0.005)
      throw new BadRequestException('journal is not balanced');
    return this.prisma.$transaction(async (tx) => {
      const created: any[] = [];
      for (const l of lines) {
        if (n(l.debit) > 0 === n(l.credit) > 0)
          throw new BadRequestException(
            'each journal line must have exactly one side',
          );
  ...
  createCampaign(org: string, b: any) {
    const name = s(b.name);
    if (!name) throw new BadRequestException('name is required');
    return this.prisma.$queryRawUnsafe(
      ...
      JSON.stringify(b.audienceFilter ?? {}),
  ...
  async runCampaign(org: string, id: string) {
    ...
    for (const r of rows) {
      ...
      try {
        if (camp[0].channel === 'WHATSAPP' || camp[0].channel === 'SMS')
          await this.communications.sendAdHoc({
            organizationId: org,
            channel: camp[0].channel,
            category: 'MARKETING',
            recipient,
        ...
        await this.prisma.$executeRawUnsafe(
          "UPDATE marketing_campaign_members SET status='SENT',sent_at=now() WHERE campaign_id=$1 AND member_id=$2",
          id,
          r.member_id,
        );
        sent++;
        ...
        failed++;
        await this.prisma.$executeRawUnsafe(
          "UPDATE marketing_campaign_members SET status='FAILED',error=$3 WHERE campaign_id=$1 AND member_id=$2",
          id,
          r.member_id,
    ...
    await this.prisma.$executeRawUnsafe(
      "UPDATE marketing_campaigns SET status=CASE WHEN $2=0 THEN 'COMPLETED' ELSE 'RUNNING' END,updated_at=now() WHERE id=$1",
      id,
      rows.length,
    );
  ...
  async registerKiosk(org: string, userId: string, b: any) {
    ...
    await this.audit.record({
      ...
      afterState: { branchId: b.branchId, name: b.name },
    });
