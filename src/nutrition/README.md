...
Diet/meal plans, macro targets, food database, versioned plan history. See docs/ARCHITECTURE.md#diet-nutrition-engine.
...
- Routes protected by `@RequirePermissions('nutrition.<action>')` against the permission catalog in `src/rbac/permissions.catalog.ts` (already seeded).
