...
export class WorkoutAssignmentsController {
  constructor(
    ...
    private readonly workoutExecutionService: WorkoutExecutionService,
  ) {}

  @Get('today-sessions')
  @RequirePermissions('workouts.read')
  todaysSessions(@CurrentUser() user: AuthenticatedUser) {
    return this.workoutExecutionService.todaySessions(user.organizationId!);
  }
  ...
  startSession(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string) {
    return this.workoutExecutionService.start(user.organizationId!, id, user.id);
  }

  @Post(':id/session/:sessionId/exercises/:sessionExerciseId/sets')
  @RequirePermissions('workouts.assign')
  @Audited({ resource: 'workout_set', action: 'create' })
  logSet(
    ...
    return this.workoutExecutionService.logSet(user.organizationId!, sessionId, sessionExerciseId, dto);
  }

  @Post(':id/session/:sessionId/complete')
  @RequirePermissions('workouts.assign')
  ...
  completeSession(
    ...
    return this.workoutExecutionService.complete(user.organizationId!, sessionId, notes);
  }
