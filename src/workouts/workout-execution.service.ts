...
export class WorkoutExecutionService {
  ...
  async start(organizationId: string, assignmentId: string, userId: string) {
    const existing = await this.prisma.$queryRaw<SqlSession[]>`
      ...
      FROM workout_sessions WHERE organization_id = ${organizationId}::uuid
        AND workout_assignment_id = ${assignmentId} AND session_date = CURRENT_DATE LIMIT 1`;
    ...
    const created = await this.prisma.$queryRaw<SqlSession[]>`
      INSERT INTO workout_sessions (client_id, trainer_id, workout_assignment_id, session_date, status, created_by, organization_id)
      VALUES (${assignment[0].client_id}, ${assignment[0].trainer_id}, ${assignmentId}, CURRENT_DATE, 'in_progress', ${userId}, ${organizationId}::uuid)
      ON CONFLICT (workout_assignment_id, session_date) DO UPDATE
        SET status = CASE WHEN workout_sessions.status = 'completed' THEN workout_sessions.status ELSE 'in_progress' END,
            updated_at = now()
  ...
  async logSet(organizationId: string, sessionId: string, sessionExerciseId: string, dto: LogWorkoutSetDto) {
    ...
    const rows = await this.prisma.$queryRaw`
      INSERT INTO workout_sets (session_exercise_id, set_number, weight_kg, reps, rpe, rir, tempo, rest_seconds, completed, notes, client_token)
      VALUES (${sessionExerciseId}, ${dto.setNumber}, ${dto.weightKg ?? null}, ${dto.reps ?? null}, ${dto.rpe ?? null}, ${dto.rir ?? null}, ${dto.tempo ?? null}, ${dto.restSeconds ?? null}, ${dto.completed ?? false}, ${dto.notes ?? null}, ${dto.clientToken ?? null})
