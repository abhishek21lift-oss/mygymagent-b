...
export class FileStorageService {
  ...
  async upload(input: UploadFileInput): Promise<UploadFileResult> {
    ...
    // Validate MIME type -- against the sniffed content, not just the
    // client-claimed multipart type (see sniffMimeType).
    const sniffed = sniffMimeType(input.buffer);
    if (
      !FileStorageService.ALLOWED_MIME_TYPES.includes(input.mimeType as any) ||
      sniffed !== input.mimeType
    ) {
      throw new BadRequestException(
...
/**
 * Sniffs the true content type from magic bytes. The multipart `mimetype`
 * is a client claim (curl can send anything) -- without this, a renamed
 * executable/script passes the allowlist and lands in the bucket behind a
 * trusted signed URL.
 */
