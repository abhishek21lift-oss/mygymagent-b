...
export class WhatsAppService {
  ...
  private graphVersion(): string { return this.config.get<string>('WHATSAPP_GRAPH_VERSION', 'v25.0'); }
  private metaAppId(): string { return this.config.get<string>('META_APP_ID') ?? ''; }
  private metaAppSecret(): string { return this.config.get<string>('META_APP_SECRET') ?? ''; }
  ...
  async completeEmbeddedSignup(organizationId: string, input: EmbeddedSignupInput) {
    ...
    if (!this.metaAppId() || !this.metaAppSecret()) throw new ServiceUnavailableException('Meta app credentials are not configured');
    if (!input.code || !input.wabaId) throw new BadRequestException('Embedded Signup did not return an authorization code and WABA ID');
  ...
  async disconnect(organizationId: string) {
    ...
    if (integration?.encrypted_access_token && integration.waba_id && this.encryptionKey()) {
      try { await this.unsubscribeWaba(integration.waba_id, decryptWhatsAppToken(integration.encrypted_access_token, this.encryptionKey())); } catch { /* already revoked */ }
    }
    await this.prisma.$executeRaw(Prisma.sql`
  ...
  private async fetchWabaPhones(wabaId: string, token: string) {
    const response = await fetch(`https://graph.facebook.com/${this.graphVersion()}/${wabaId}/phone_numbers?fields=id,display_phone_number,verified_name`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const body = await response.json() as { data?: Array<{ id: string; display_phone_number?: string; verified_name?: string }>; error?: { message?: string } };
    if (!response.ok) throw new BadRequestException(body.error?.message ?? 'Unable to verify WhatsApp Business Account with Meta');
    return (body.data ?? []).map((item) => ({ id: item.id, displayPhoneNumber: item.display_phone_number, displayName: item.verified_name }));
  }

  private async subscribeWaba(wabaId: string, token: string) {
    const response = await fetch(`https://graph.facebook.com/${this.graphVersion()}/${wabaId}/subscribed_apps`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
  ...
  private async unsubscribeWaba(wabaId: string, token: string) {
    await fetch(`https://graph.facebook.com/${this.graphVersion()}/${wabaId}/subscribed_apps`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
  }
