...
interface WebhookEntry { id?: string; changes?: WebhookChange[]; }
...
export class WhatsAppService {
  ...
  /**
   * Completes Meta's code-based Embedded Signup. The short-lived code never
   * gets persisted; it is exchanged server-side for the customer token,
   * verified against the selected WABA/phone number, subscribed to webhooks,
   * and then encrypted at rest.
   */
  async completeEmbeddedSignup(organizationId: string, input: EmbeddedSignupInput) {
    if (!this.encryptionKey()) {
      throw new ServiceUnavailableException('WhatsApp encryption is not configured');
    }
    ...
    if (!input.code || !input.wabaId || !input.phoneNumberId) {
      throw new BadRequestException('Embedded Signup did not return a code, WABA ID, and phone number ID');
    }

    const token = await this.exchangeSignupCode(input.code);
    const phone = await this.fetchPhoneNumber(input.phoneNumberId, token);
    const phoneIds = await this.fetchWabaPhoneIds(input.wabaId, token);
    if (!phoneIds.includes(input.phoneNumberId)) {
      throw new BadRequestException('Selected phone number does not belong to the selected WhatsApp Business Account');
    }
  ...
  async connect(organizationId: string, input: ConnectInput) {
    ...
    return this.persistConnection(organizationId, {
      ...
      displayPhoneNumber: input.displayPhoneNumber ?? verified.displayPhoneNumber,
    ...
    if (existing) {
      ...
      await this.prisma.$executeRaw(Prisma.sql`
        INSERT INTO whatsapp_integrations
          (id, organization_id, waba_id, phone_number_id, business_account_id,
           display_phone_number, display_name, encrypted_access_token, status, last_verified_at)
        VALUES
          (${randomUUID()}, ${organizationId}, ${input.wabaId ?? null}, ${input.phoneNumberId},
           ${input.businessAccountId ?? null}, ${input.displayPhoneNumber ?? null},
           ${input.displayName ?? null}, ${encrypted}, 'CONNECTED', CURRENT_TIMESTAMP)
      `);
    }
  ...
  async disconnect(organizationId: string) {
    const rows = await this.prisma.$queryRaw<(IntegrationRow & { encrypted_access_token: string })[]>(Prisma.sql`
      SELECT id, organization_id, waba_id, phone_number_id, business_account_id,
             display_phone_number, display_name, status, last_verified_at, encrypted_access_token
      FROM whatsapp_integrations WHERE organization_id=${organizationId} LIMIT 1
    `);
    const integration = rows[0];
    if (integration?.encrypted_access_token && integration.waba_id && this.encryptionKey()) {
      try {
        const token = decryptWhatsAppToken(integration.encrypted_access_token, this.encryptionKey());
        await this.unsubscribeWaba(integration.waba_id, token);
      } catch {
        // Local disconnect still succeeds if Meta has already revoked access.
      }
    }
    await this.prisma.$executeRaw(Prisma.sql`
      UPDATE whatsapp_integrations
  ...
  async sendText(organizationId: string, to: string, text: string) {
    ...
    const token = decryptWhatsAppToken(integration.encrypted_access_token, this.encryptionKey());
    const response = await fetch(`https://graph.facebook.com/${this.graphVersion()}/${integration.phone_number_id}/messages`, {
      method: 'POST',
    ...
    await this.prisma.$executeRaw(Prisma.sql`
      INSERT INTO whatsapp_messages
        (id, organization_id, phone_number_id, provider_message_id, direction,
         to_number, message_type, text, status, created_at, updated_at)
      VALUES
        (${randomUUID()}, ${organizationId}, ${integration.phone_number_id}, ${providerMessageId},
  ...
  webhookVerify(mode: string | undefined, token: string | undefined, challenge: string | undefined) {
    ...
    if (!expected || mode !== 'subscribe' || !token || !challenge || token.length !== expected.length || !timingSafeEqual(Buffer.from(token), Buffer.from(expected))) {
  ...
  async handleWebhook(payload: WebhookPayload) {
    ...
    for (const entry of entries) for (const change of entry.changes ?? []) {
      ...
      const rows = await this.prisma.$queryRaw<{ organization_id: string }[]>(Prisma.sql`
      ...
      for (const message of value?.messages ?? []) {
        ...
        await this.prisma.$executeRaw(Prisma.sql`
          INSERT INTO whatsapp_messages
            (id, organization_id, phone_number_id, provider_message_id, direction,
             from_number, to_number, message_type, text, status, raw_payload, created_at, updated_at)
          VALUES
            (${randomUUID()}, ${organizationId}, ${phoneNumberId}, ${message.id ?? null}, 'INBOUND',
             ${message.from ?? null}, ${phoneNumberId}, ${message.type ?? 'unknown'}, ${text},
             'RECEIVED', ${JSON.stringify(message)}::jsonb, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      ...
      for (const status of value?.statuses ?? []) {
        await this.prisma.$executeRaw(Prisma.sql`
          ...
          SET status=${String(status.status ?? 'UNKNOWN').toUpperCase()}, updated_at=CURRENT_TIMESTAMP
  ...
  private async exchangeSignupCode(code: string): Promise<string> {
    ...
    if (!response.ok || !body.access_token) throw new BadRequestException(body.error?.message ?? 'Meta Embedded Signup code exchange failed');
    return body.access_token;
  }

  private async fetchPhoneNumber(phoneNumberId: string, token: string) {
    const url = `https://graph.facebook.com/${this.graphVersion()}/${phoneNumberId}?fields=display_phone_number,verified_name`;
    const response = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
    const body = await response.json() as { display_phone_number?: string; verified_name?: string; error?: { message?: string } };
    if (!response.ok) throw new BadRequestException(body.error?.message ?? 'Unable to verify WhatsApp phone number with Meta');
    return { displayPhoneNumber: body.display_phone_number, displayName: body.verified_name };
  }

  private async fetchWabaPhoneIds(wabaId: string, token: string) {
    const url = `https://graph.facebook.com/${this.graphVersion()}/${wabaId}/phone_numbers?fields=id,display_phone_number,verified_name`;
    const response = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
    const body = await response.json() as { data?: Array<{ id: string }>; error?: { message?: string } };
    if (!response.ok) throw new BadRequestException(body.error?.message ?? 'Unable to verify WhatsApp Business Account with Meta');
    return (body.data ?? []).map((item) => item.id);
  }

  private async subscribeWaba(wabaId: string, token: string) {
    const response = await fetch(`https://graph.facebook.com/${this.graphVersion()}/${wabaId}/subscribed_apps`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
    });
    const body = await response.json() as { success?: boolean; error?: { message?: string } };
    if (!response.ok || body.success === false) throw new BadRequestException(body.error?.message ?? 'Unable to subscribe WhatsApp Business Account to MyGymAgent webhooks');
  }

  private async unsubscribeWaba(wabaId: string, token: string) {
    await fetch(`https://graph.facebook.com/${this.graphVersion()}/${wabaId}/subscribed_apps`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
  }
  ...
  private async verifyToken(phoneNumberId: string, accessToken: string) {
    return this.fetchPhoneNumber(phoneNumberId, accessToken);
  }

  async listMessages(organizationId: string, limit = 100) {
    const safeLimit = Math.min(Math.max(Number(limit) || 100, 1), 200);
    return this.prisma.$queryRaw(Prisma.sql`
      SELECT id, phone_number_id, provider_message_id, direction, from_number,
             to_number, message_type, text, status, created_at, updated_at
