...
export class PlatformBillingService {
  ...
  async subscription(org: string) {
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT s.*,p."key" AS "planKey",p.name AS "planName",p."maxMembers",p."maxBranches",p."maxStaff",p."aiMonthlyRequests",p."storageMb",p."whatsappMonthly",p."apiMonthlyCalls" FROM organization_subscriptions s JOIN subscription_plans p ON p.id=s."planId" WHERE s."organizationId"=$1 LIMIT 1`,
      org,
    );
    return rows[0] ?? null;
  }
  ...
  async usage(org: string) {
    const [members, branches, staff] = await Promise.all([
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT count(*)::int AS count FROM members WHERE "organizationId"=$1 AND COALESCE("deletedAt",NULL) IS NULL`,
        org,
      ),
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT count(*)::int AS count FROM branches WHERE "organizationId"=$1 AND COALESCE("deletedAt",NULL) IS NULL`,
        org,
      ),
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT count(*)::int AS count FROM users WHERE "organizationId"=$1 AND COALESCE("deletedAt",NULL) IS NULL`,
        org,
      ),
    ]);
