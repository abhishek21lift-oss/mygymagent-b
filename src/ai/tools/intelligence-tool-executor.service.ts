  ...
  async execute(
    ...
    const previousDate =
      sessionDates.length > 1
        ? sessionDates[sessionDates.length - 2]
        : undefined;
    const latestRows = ordered.filter(
      (row) =>
        new Date(row.session_date).toISOString().slice(0, 10) === latestDate,
    );
    ...
    const avgVolume = (items: typeof ordered) =>
      items.reduce(
        (sum, row) => sum + (row.weight_kg ?? 0) * (row.reps ?? 0),
        0,
      ) / Math.max(items.length, 1);

    const latestAverageVolume = avgVolume(latestRows);
    const previousAverageVolume = previousRows.length
      ? avgVolume(previousRows)
      : null;
    const changePercent = previousAverageVolume
      ? ((latestAverageVolume - previousAverageVolume) /
          previousAverageVolume) *
        100
      : null;

    let signal: 'progressing' | 'regressing' | 'stable' | 'insufficient_data' =
      'stable';
    ...
    return {
      ...
      previousAverageVolume:
        previousAverageVolume === null
          ? null
          : Math.round(previousAverageVolume * 100) / 100,
      changePercent:
        changePercent === null ? null : Math.round(changePercent * 100) / 100,
      bestRecordedSet: {
