...
export class ToolExecutorService {
  ...
  private async readMember(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) {
    ...
    const assignmentScope = matchedKey === 'members.read_assigned' ? userId : null;
    const member = await this.membersService.getOne(organizationId, memberId, branchScope, assignmentScope);
  ...
  private async readWorkoutHistory(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) {
    ...
    const assignmentScope = matchedKey === 'workouts.read_assigned' ? userId : null;
    const assignments = await this.workoutAssignmentsService.list(organizationId, { page: 1, pageSize: 20 }, memberId, branchScope, assignmentScope);
  ...
  private async createWorkoutDraft(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { await this.resolveAccess(userId, organizationId, requestedBranchId, ['workouts.create']); const dto = validateToolArgs(CreateWorkoutPlanDto, rawArgs); const plan = await this.workoutPlansService.create(organizationId, dto, userId); await this.audit.record({ organizationId, actorUserId: userId, action: 'ai_tool.create_workout_draft', resource: 'workout_plan', resourceId: plan.id, afterState: { name: plan.name, exerciseCount: dto.exercises.length } }); return { id: plan.id, name: plan.name }; }
  private async createDietDraft(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { await this.resolveAccess(userId, organizationId, requestedBranchId, ['nutrition.create']); const dto = validateToolArgs(CreateDietPlanDto, rawArgs); const plan = await this.dietPlansService.create(organizationId, dto, userId); await this.audit.record({ organizationId, actorUserId: userId, action: 'ai_tool.create_diet_draft', resource: 'diet_plan', resourceId: plan.id, afterState: { name: plan.name, itemCount: dto.items.length } }); return { id: plan.id, name: plan.name }; }
  private async createFollowup(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { const { leadId, note, dueAt } = validateToolArgs(CreateFollowupArgsDto, rawArgs); const { branchScope } = await this.resolveAccess(userId, organizationId, requestedBranchId, ['leads.manage']); const followUp = await this.leadsService.addFollowUp(organizationId, leadId, { note, dueAt }, userId, branchScope); await this.audit.record({ organizationId, actorUserId: userId, action: 'ai_tool.create_followup', resource: 'lead_follow_up', resourceId: followUp.id, afterState: { leadId, note, dueAt } }); return { id: followUp.id, dueAt: followUp.dueAt }; }
  private async getRevenueSummary(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { validateToolArgs(EmptyArgsDto, rawArgs); const { branchScope } = await this.resolveAccess(userId, organizationId, requestedBranchId, ['reports.view']); return this.financeService.getRevenueSummary(organizationId, {}, branchScope); }
  private async getAtRiskMembers(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { validateToolArgs(EmptyArgsDto, rawArgs); const { branchScope } = await this.resolveAccess(userId, organizationId, requestedBranchId, ['reports.view']); return this.memberIntelligence.getAtRiskMembers(organizationId, branchScope); }
  private async getSalesFunnel(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { validateToolArgs(EmptyArgsDto, rawArgs); const { branchScope } = await this.resolveAccess(userId, organizationId, requestedBranchId, ['reports.view']); return this.salesIntelligence.getFunnel(organizationId, branchScope, {}); }
  private async getTrainerWorkload(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { validateToolArgs(EmptyArgsDto, rawArgs); const { branchScope } = await this.resolveAccess(userId, organizationId, requestedBranchId, ['reports.view']); return this.trainerIntelligence.getWorkload(organizationId, branchScope); }
  private async getInventoryForecast(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { validateToolArgs(EmptyArgsDto, rawArgs); await this.resolveAccess(userId, organizationId, requestedBranchId, ['reports.view']); return this.inventoryIntelligence.getStockForecast(organizationId); }
  private async getDailyBriefing(rawArgs: unknown, { organizationId, userId, requestedBranchId }: ToolCallContext) { validateToolArgs(EmptyArgsDto, rawArgs); const { branchScope } = await this.resolveAccess(userId, organizationId, requestedBranchId, ['reports.view']); return this.dailyBriefingService.getBriefing(organizationId, branchScope); }
  private async proposeAssignWorkoutPlan(rawArgs: unknown, context: ToolCallContext) { await this.resolveAccess(context.userId, context.organizationId, context.requestedBranchId, ['workouts.assign']); const payload = validateToolArgs(AssignPlanPayloadDto, rawArgs); return this.aiActionsService.proposeAssignWorkoutPlan(context.organizationId, context.userId, payload); }
  private async proposeAssignDietPlan(rawArgs: unknown, context: ToolCallContext) { await this.resolveAccess(context.userId, context.organizationId, context.requestedBranchId, ['nutrition.assign']); const payload = validateToolArgs(AssignPlanPayloadDto, rawArgs); return this.aiActionsService.proposeAssignDietPlan(context.organizationId, context.userId, payload); }
}
