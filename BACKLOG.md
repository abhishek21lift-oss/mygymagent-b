...
| B-P0-2 | e2e test coverage for untested modules | 🚧 | **`business-os` done**: `test/business-os.e2e-spec.ts` (new, 38 tests) covers every route — loyalty (incl. a concurrency regression test: 8 parallel point-adjustment requests against the same account under the `FOR UPDATE` lock land exactly, proving no lost update), referrals (incl. the loyalty-credit-on-convert branch), support tickets (incl. a regression test for `resolvedAt` surviving a RESOLVED→CLOSED transition), feedback (NPS math), marketing campaigns (the `enrollCampaign` audience-filter bug B-P0-1 fixed — this is the test that proves it now actually enrolls the right members, where before it 500'd on every call), accounting (balanced/unbalanced journals, trial balance, tax summary), member portal (bootstrap, single-use consumption, revocation, and the rate limiter's 429 after 20 requests), kiosk (check-in allow/deny paths, branch mismatch, rate limiting), and a `TRAINER`-role 403 check per permission group. Full suite verified green against a real, locally-run Postgres+Redis+s3rver+SMTP-capture (26 suites, 194 passed + 3 pre-existing skips, zero regressions). **Still open**: `hr-payroll`, `classes`/group training, notification center, WhatsApp send path — same zero-coverage gap, not yet started. |
