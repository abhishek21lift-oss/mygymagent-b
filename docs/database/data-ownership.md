...
| `MemberAddress` / `MemberEmergencyContact` | Organization (about a Member) | `organizationId` | Inherited via `Member.primaryBranchId` (no own branch column) | Org/branch staff per `members.read`/`read_assigned` | Staff with `members.update` | No — ordinary address-book data, not financial/compliance history; rows are hard-deletable | Yes (delete allowed, not soft) | Yes (`@Audited`) | Not yet wired to a tool — plain member-profile data, same boundary as `Member` itself once exposed | Yes, nested under `/members/:id/...` |
| `MemberNote` | Organization (about a Member) | `organizationId` | Inherited via Member | Org/branch staff per `members.read`/`read_assigned` | Author only may edit/delete their own note (service-enforced, not just UI) | Yes — a collection, never overwritten (this table exists specifically to replace the old single overwritable `Member.notes` field) | No (author-only delete) | Yes | Not yet wired to a tool; free-text staff notes are a natural future prompt-injection surface if ever fed to a model — treat as untrusted content per `docs/ai/architecture.md` when that happens | Yes |
| `MemberConsent` | Organization (about a Member) | `organizationId` | Inherited via Member | Org/branch staff per `members.read`/`read_assigned` | Staff with `members.update` (`record`, never edit/delete) | **Yes — compliance record.** Append-only; revoking creates a new row, never mutates the grant | **No — never deleted or updated** | Yes | No | Yes, `POST`/`GET` only (no `PATCH`/`DELETE` route exists) |
| `MemberStatusHistory` / `MemberBranchHistory` / `MemberTrainerHistory` | Organization (about a Member) | `organizationId` | `MemberBranchHistory` also carries `fromBranchId`/`toBranchId` | Org/branch staff per `members.read`/`read_assigned` | Written only by `MembersService.create()`/`update()` — no direct write endpoint | **Yes — this is the historical record itself**, closing the gap flagged in `docs/architecture/discovery-report.md` §6 ("changing a trainer/status/branch must not destroy history") | No — append-only, never deleted or updated | Not separately audited — the row itself is the record, same reasoning as `Attendance` | No | Yes, read-only (`GET /members/:id/status-history` etc.) |

**Known backlog**: this matrix was kept current through the deep-foundation phase and the Member
360 work above, but was not backfilled when `billing`/`workouts`/`nutrition`/`inventory`/`crm`/`ai`
were built — those tables' ownership answers exist implicitly in their own service code and
`docs/security/overview.md`, not here yet. Flagging honestly rather than leaving it looking
complete.
