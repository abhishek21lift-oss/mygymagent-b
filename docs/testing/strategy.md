...
| Integration/E2E | Jest + Supertest, real Postgres **and real Redis** (`npm run test:e2e`) | 103 tests across 16 suites — `auth`, `tenant-isolation`, `branch-scoping`, `member-assignment-scoping`, `rate-limiting`, `platform`, `ai`, `member-360`, `member-assessments-goals`, `notifications-queue`, `app` (health), and the core domain modules. Runs against a real database (migrated + seeded fresh each run via `test/global-setup.ts`) and a real Redis (CI provisions both as service containers, see `.github/workflows/ci.yml`) — not mocks: tenant-isolation bugs live in the interaction between the Prisma `where` clause and the DB, and the queue tests assert a job actually completes via a real BullMQ worker, neither of which a mock would catch. |
| Security | Same e2e suite | See `docs/security/overview.md`'s test matrix for the exact checklist and what's not yet covered (upload safety — no `files` module yet — and prompt injection against a live model). |
