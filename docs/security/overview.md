...
| Second factor (TOTP) on staff login | ✅ (added 2026-09-22) `test/auth-mfa.e2e-spec.ts` + `src/auth/mfa/mfa-secret.vault.spec.ts`. Proves the properties that make 2FA more than decorative: a correct password alone yields no session and no refresh cookie once enrolled; the `mfa`-typed challenge token is refused as a bearer credential (so it can't be swapped for a session); a code can't be replayed inside its own 30s step; recovery codes are single-use; wrong codes burn the *same* lockout budget as wrong passwords (a 6-digit keyspace is otherwise brute-forceable); and a merely *pending* enrolment never gates login. Secrets are stored only as AES-256-GCM envelopes (`MFA_TOTP_KEY`), never in plaintext. Enrolment is opt-in per user by default; an organization can require it — see the row below. |
| Issuing physical entry is separated from reading a profile | ✅ (fixed 2026-09-23) `test/entry-credential-privilege.e2e-spec.ts`. The QR-mint route accepted `members.read`, so an accountant, a sales executive or a nutritionist could produce a working turnstile credential for any member — minting durable building access was a lower bar than recording one check-in with it. Now `attendance.create*` only. The three affected roles keep their member-read access; every door-working role was already covered, so no role definition changed. |
| Rate limiting actually applies at the documented rate | ✅ (fixed 2026-09-23) `src/common/rate-limit/throttler.config.spec.ts`. The global throttler listed five **unnamed** configs; `@nestjs/throttler` names each of those `'default'`, so they shared one counter and each incremented it — one request counted as five hits against the strictest limit, making the advertised 120/min an effective ~4/min. That is a self-inflicted denial of service, not a tightening: real sessions hit 429 within seconds, and the only reason it went unnoticed is that the e2e suite bypasses throttling via TEST_MODE. Now one unnamed entry at 120/min, with per-area limits on the controllers where they already were, and a unit test that fails if a second unnamed entry returns. |
| Enforced second factor for privileged roles | ✅ (added 2026-09-23) `test/auth-mfa-policy.e2e-spec.ts`. An organization can set `MfaPolicy.REQUIRED_FOR_PRIVILEGED`, covering `ORG_OWNER`/`ORG_ADMIN`/`ACCOUNTANT`. Enforcement **never refuses the login** — that would lock a privileged user out of the only screen that fixes it. It issues a session confined by `MfaEnrolmentGuard` (global, deny-by-default) to `/auth/me`, `/auth/logout-all` and the three enrolment routes; everything else 403s until they enrol. The restriction is recomputed per request from the DB, so completing enrolment lifts it on the next call with no new token. A future `mfaGraceUntil` downgrades this to a warning carried on both the login response and `/auth/me`. Defaults to `OPTIONAL`, so existing tenants are unaffected until an admin acts. The enrolment report (`GET /auth/mfa/policy/report`) is gated on `organizations.update`, not `users.read`: it names which privileged accounts are unprotected. |
| Trainer → unauthorized member (a trainer reading a member not assigned to them) | ✅ for the Members module — `test/member-assignment-scoping.e2e-spec.ts`: a TRAINER's member list/detail endpoints only return members where `assignedTrainerId` is their own id; a broad `members.read` holder (e.g. the org owner) is unaffected. NUTRITIONIST is a documented exception, not covered — see the RBAC section above. **Gap closed 2026-09-22** (was: "`/memberships/:id`, `/attendance`, and `/workout-assignments` do not apply assignment scoping at all"). `test/assignment-scoping-rest.e2e-spec.ts` now covers that surface route by route. Verifying it found the remediation had been only half applied: the list/detail/check-in/check-out routes were scoped, but five sibling routes were gated on a `*_assigned` permission while never applying the scope — `/memberships/analytics/summary`, `/memberships/renewal-reminders`, `/memberships/history/:id`, `/attendance/live`, and `/attendance/qr-token/:memberId`, the last of which **minted a working QR entry credential for any member in the org**. All five are fixed, and reverting the fixes makes exactly those five test cases fail, so the coverage is known to detect the regression rather than merely coexist with it. |
